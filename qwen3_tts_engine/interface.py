...
from qwen3_tts_engine.utils.prompt import prepare_custom_voice_prompt, _ensure_list, _tokenize_texts
from qwen3_tts_engine.processor import Qwen3TTSProcessor
from qwen3_tts_engine.utils.generation import prepare_inputs, generate_speaker_prompt, generate_icl_prompt
from qwen3_tts_engine.llm import TalkerLLM, PredictorLLM
...
try:
    from qwen3_tts_engine.utils.audio import SpeechTokenizer
...
try:
    from qwen3_tts_engine.utils.speech_tokenizer_cudagraph import SpeechTokenizerCUDAGraph
    HAS_CUDAGRAPH_TOKENIZER = True
except ImportError:
    HAS_CUDAGRAPH_TOKENIZER = False
    SpeechTokenizerCUDAGraph = None
...
class Qwen3TTSInterface:
    ...
    def from_pretrained(
        ...
        *,
        ...
        zmq_bridge=None,
        talker_gpu_mem: float = 0.3,
        predictor_gpu_mem: float = 0.5,
    ):
        ...
        return cls(
            ...
            talker_gpu_mem=talker_gpu_mem,
            predictor_gpu_mem=predictor_gpu_mem,
        )
    
    def __init__(self, model_path: str, enforce_eager: bool = False, tensor_parallel_size: int = 1, zmq_bridge=None,
                 talker_gpu_mem: float = 0.3, predictor_gpu_mem: float = 0.5):
        self.model_path = model_path
        self.enforce_eager = enforce_eager
        self.tensor_parallel_size = tensor_parallel_size
        self.zmq_bridge = zmq_bridge
        self.talker_llm = TalkerLLM(model_path, enforce_eager=enforce_eager, tensor_parallel_size=tensor_parallel_size, gpu_memory_utilization=talker_gpu_mem)
        self.predictor_llm = PredictorLLM(model_path, enforce_eager=enforce_eager, tensor_parallel_size=tensor_parallel_size, gpu_memory_utilization=predictor_gpu_mem)
    ...
    def _init_speech_components(self):
        ...
        try:
            # Prefer CUDAGraph-optimized tokenizer for faster decode
            if HAS_CUDAGRAPH_TOKENIZER:
                self.speech_tokenizer = SpeechTokenizerCUDAGraph(
                    "Qwen/Qwen3-TTS-Tokenizer-12Hz", dtype=torch.bfloat16,
                )
            elif HAS_SPEECH_TOKENIZER:
    ...
    def prepare_voice_clone_inputs(
        ...
        """Prepare talker/predictor inputs for voice clone generation without running the loop.

        This extracts the input-preparation logic from generate_voice_clone so that
        multiple requests can be prepared independently and then batched together
        via _generate_batched().

        Returns:
            dict with keys: inputs_embeds, trailing_text_hiddens, tts_pad_embed.
        """
        if language is None:
            language = "Auto"

        # Determine ref_text_for_ids
        icl_mode_enabled = voice_clone_prompt.get("icl_mode", False)
        if icl_mode_enabled:
            ref_text_for_ids = voice_clone_prompt.get("ref_text")
            if ref_text_for_ids is None:
                raise ValueError("ICL mode requires ref_text in voice_clone_prompt")
        else:
        ...
        # Wrap prompt values in lists (prepare_inputs expects batch format)
        ...
        def generate_icl_prompt_fn(
            text_id, ref_id, ref_code, tts_pad_embed, tts_eos_embed, non_streaming_mode
        ):
    ...
    @torch.inference_mode()
    def prepare_custom_voice_inputs(
        self,
        text: str,
        language: str = "Korean",
        speaker: str = "Vivian",
        ...
        """Prepare talker/predictor inputs for built-in speaker TTS (no voice clone).

        Returns same format as prepare_voice_clone_inputs so batched engine can use it.
        """
    ...
    def generate_voice_clone(
        ...
        yield from self._generate_caller_driven(
            ...
            str(uuid.uuid4()),
            SamplingParams(temperature=0.8, max_tokens=1, repetition_penalty=1.05),
            SamplingParams(temperature=0.2, max_tokens=17),
        )
    ...
    async def start_zmq_tasks(self) -> None:
        ...
        from qwen3_tts_engine.zmq.dispatcher import start_dispatcher_thread, run_dispatch_loop
        from qwen3_tts_engine.zmq.engine_loop import run_engine_loop
    ...
    def generate(self, inputs_embeds: torch.Tensor, trailing_text_hiddens: torch.Tensor, tts_pad_embed: torch.Tensor, talker_attention_mask: torch.Tensor, request_id: str | None = None):
        ...
        talker_sampling_params = SamplingParams(temperature=0.8, max_tokens=1, repetition_penalty=1.05)
        predictor_sampling_params = SamplingParams(temperature=0.2, max_tokens=17)
        yield from self._generate_caller_driven(
    ...
    def _generate_batched(
        ...
        """Process multiple TTS requests in parallel using GPU batching.

        Each talker step() and predictor step() call processes ALL active
        sequences in one GPU kernel launch, giving near-linear throughput
        scaling with batch size.

        Args:
            batch_items: list of dicts, each with keys:
                - request_id: str
                - inputs_embeds: torch.Tensor  [1, seq_len, D] or [seq_len, D]
                - trailing_text_hiddens: torch.Tensor  [1, T, D]
                - tts_pad_embed: torch.Tensor

        Returns:
            dict mapping request_id → list of codebook_id chunks
            (each chunk is [c0, c1, …, c15] for one codec timestep).
        """
        ...
        for item in batch_items:
            ...
            active[rid] = {
                "trailing_text_hiddens": item["trailing_text_hiddens"],
                "tts_pad_embed": item["tts_pad_embed"],
                "generation_step": 0,
            }
            results[rid] = []
            # Enqueue for prefill
            self.talker_llm.add_request([embeds], talker_sp, request_id=rid)

        _iter = 0
        while active:
            _iter += 1
            # ── Talker step (single GPU call for all active sequences) ──
            _, _, outputs_all = self.talker_llm.step_with_outputs()
            if not outputs_all:
                print(f"[batched] iter={_iter}: no outputs, breaking")
                break

            finished_rids: List[str] = []
            predictor_items: List[dict] = []

            for rid, _seq_id, token_ids, hidden_states, _is_finished in outputs_all:
                if rid not in active:
                    continue
                last_id = token_ids[-1]
                if _iter <= 3 or _iter % 50 == 0:
                    print(f"[batched] iter={_iter} rid={rid[:8]} last_id={last_id} n_tokens={len(token_ids)} finished={_is_finished}")
                if last_id == 2150:  # EOS
                    self.talker_llm.clear_request(rid)
                ...
                last_hidden_state = hidden_states.unsqueeze(0).unsqueeze(0)
                pred_embeds = torch.cat((last_hidden_state, last_id_hidden), dim=1)

                predictor_items.append({
                    "rid": rid,
                    "last_id": last_id,
            ...
            if not predictor_items:
                continue

            # ── Predictor: use generate() per item (same path as serial) ──
            for pi in predictor_items:
                rid = pi["rid"]
                if rid not in active:
                    continue

                predictor_outputs = self.predictor_llm.generate(
                    [pi["pred_embeds"].unsqueeze(0)],
                    predictor_sp,
                    use_tqdm=False,
                    request_id=rid,
                )
                pred_token_ids = predictor_outputs[0]["token_ids"]
                codebook_ids = [pi["last_id"]] + pred_token_ids
                results[rid].append(codebook_ids)

                # Build next talker embeds
                last_id_hidden = pi["last_id_hidden"]
                codec_hiddens = torch.cat(
                    [last_id_hidden]
                    + [
                ...
                if gen_step < state["trailing_text_hiddens"].shape[1]:
                    next_embeds = next_embeds + state["trailing_text_hiddens"][:, gen_step].unsqueeze(1)
                else:
                    next_embeds = next_embeds + state["tts_pad_embed"]
                state["generation_step"] = gen_step + 1

                # Set decode_input_embeds for next talker step
                self.talker_llm.add_request([next_embeds], talker_sp, request_id=rid)

        return results

    def create_batched_engine(self, max_concurrent: int = 64):
        """Create a BatchedStreamingEngine for concurrent streaming TTS.

        The engine uses this interface's single Talker + Predictor instance
        to serve N concurrent requests via batched step() calls.
        Call .start() on the returned engine to begin the background loop.
        """
        from qwen3_tts_engine.batched_engine import BatchedStreamingEngine
        return BatchedStreamingEngine(self, max_concurrent=max_concurrent)
